...
typedef struct NetworkResponse{ // define network response between node and miner
    ...
    uint64_t height; // height of block
}NETWORKRESPONSE;

typedef struct knownNode{ // defining address of a node
    char nodeName[36];
    char nodeService[8]; // port  number and service ,,,,,,
}KNOWNNODE;
